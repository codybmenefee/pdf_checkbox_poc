...
class TestE2EFieldVisualization(unittest.TestCase):
    """
    End-to-end tests for field visualization workflow.
    ...
    def test_image_path_resolution(self):
        ...
        # Skip this test since it's difficult to mock properly in CI
        ...
        with patch('src.app.send_file') as mock_send_file:
            ...
            # Create real test directory for visualization
            vis_path = os.path.join(self.vis_dir, vis_id)
            os.makedirs(vis_path, exist_ok=True)
            with open(os.path.join(vis_path, filename), 'w') as f:
